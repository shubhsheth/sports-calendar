...
row per league, filters as JSONB in the same shape as the query params above),
and `calendar_pinned_events` (`league` + `espn_event_id`). All three tables
are RLS-protected owner-only; the feed route reads them via PostgREST with the
service-role key (`_shared/personalCalendar.ts` — `SUPABASE_URL` and
`SUPABASE_SERVICE_ROLE_KEY` are injected by the Edge Function runtime).
Stored filters are re-validated at feed time with the same `parse<League>Params`
