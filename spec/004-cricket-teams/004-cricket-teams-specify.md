...
The home page stays a pure **selector** — no events on it. A segmented chip control at
the top switches between **Leagues** and **Teams**: Leagues shows today's league tile
grid unchanged; Teams shows a matching grid of the 12 curated ICC full members (logo +
name). A team tile navigates to that team's dedicated page (`/cricket-teams/{teamId}`),
which has full league-page parity: chronological schedule across every discovered
series, format filters, one-time `.ics` download, live subscription feed links, and
Save/Pin to My Calendar. Existing per-league pages are unchanged.
...
- As a Team India fan, I want to pick India from the home page's Teams tab and land on
  one page showing every upcoming India match across all tours and tournaments so that
  I don't have to know which series exist.
...
- As a signed-in user, I want to save my team (and pin individual marquee matches) to
  My Calendar so that it combines with my existing league subscriptions in one feed.

## Functional Requirements

- FR-1: The home page gets a segmented **Leagues | Teams** chip control above the
  navigation grid (last choice persisted): Leagues shows the existing five league
  tiles unchanged; Teams shows a grid of the 12 curated national teams (logo + name)
  whose tiles navigate to `/cricket-teams/{teamId}`. Home never fetches or shows
  events.
- FR-2: A team page lists the team's matches — recent past and all discoverable
  upcoming — across every series in the discovery window, chronologically, each card
  showing opponent, series name, match format, venue, local time, and live/past
  status; unknown team IDs show a not-found state.
- FR-3: Team-page matches can be filtered by format (Test / ODI / T20I / Other) with
  the same pill-based filter UX as league pages, plus the standard show-past-events
  toggle; filters apply to the list, the download, and are encoded in feed URLs.
- FR-4: The team page offers a one-time `.ics` download of the (filtered) schedule and
  the standard Copy / Apple / Google live-feed links for that team's feed.
...
- FR-6: A signed-in user can save one or more cricket teams to My Calendar (with the
  selected format filters, from each team's page) and pin/unpin individual matches
  from the team page's cards; both appear on the My Calendar page with remove actions,
  and the personal combined feed includes them (deduplicated, like existing leagues).
...
- NFR-2a: Home issues no schedule fetches at all; a team page is one cached React
  Query entry per team, so revisiting within the stale window refetches nothing.
...
- Home layout: a segmented Leagues | Teams chip control above the grid area; the
  chosen tab (persisted in `localStorage` via the existing `useLocalStorageState`
  hook, defaulting to Leagues) decides which tile grid renders. League tiles keep
  navigating to the unchanged per-league pages; team tiles navigate to
  `/cricket-teams/{teamId}`.
- The team page fetches the whole window in one query (`fetchAllCricketTeamEvents`; a
  team plays ~30–60 matches in 7 months) — no infinite scroll; parity is in features,
  not mechanics. Default view shows upcoming events; past events behind the existing
  show-past toggle pattern.
- League pages and their card components are untouched by this feature.
...
├── cricket-team-calendar-links.tsx  → download + team feed links (+ save, T11)
└── utils/                           → feed-URL builder, filter-state toggles

client/components/home/
└── home-selector.tsx                → segmented Leagues | Teams control + team grid

client/routes/index.tsx                   → home: segmented selector + tile grids
...
- Home renders the segmented selector; the Leagues tab shows today's grid unchanged,
  the Teams tab shows all 12 team tiles, and home issues no schedule fetches on
  either tab.
- Opening India's page (team 6, via its tile) lists its matches across at least the
  currently published tours (as of authoring: Zimbabwe, Sri Lanka, West Indies, New
  Zealand series and more) sorted chronologically with correct format badges and
  series names.
...
affordances; curated 12 teams). Revised twice: after T1 the picker/team-page UX was
replaced by home-page filters over a merged schedule; after seeing that built
(2026-07-19) the human rejected events on home, settling on the final shape — home is
a segmented Leagues | Teams tile selector only, and schedules live on dedicated
`/cricket-teams/{teamId}` pages.
