...
import nbaRefs from "../fixtures/nba-refs.json";
import nbaEvent from "../fixtures/nba-event.json";
...
function makeMockFetch(): typeof fetch {
  return vi.fn(async (input: RequestInfo | URL) => {
    ...
    if (url.includes("events/401767890") && !url.includes("events/?")) {
      return jsonResponse(nbaEvent);
    }
    return jsonResponse(nbaRefs);
...
describe("GET /calendar/nba.ics", () => {
  it("returns status 200", async () => {
    const res = await app.fetch(new Request("http://localhost/calendar/nba.ics"));
  ...
  it("returns 400 for invalid showPastEvents", async () => {
    const res = await app.fetch(
      new Request("http://localhost/calendar/nba.ics?showPastEvents=invalid")
    );
  ...
  it("returns 200 for showPastEvents=false", async () => {
    const res = await app.fetch(
      new Request("http://localhost/calendar/nba.ics?showPastEvents=false")
    );
    expect(res.status).toBe(200);
  });
  ...
  it("returns 200 for valid teamIds filter", async () => {
    const res = await app.fetch(
      new Request("http://localhost/calendar/nba.ics?teamIds=13")
    );
    expect(res.status).toBe(200);
  });
  ...
  it("returns 200 for non-matching teamId (empty calendar, still valid)", async () => {
    const res = await app.fetch(
      new Request("http://localhost/calendar/nba.ics?teamIds=99999")
    );
    expect(res.status).toBe(200);
  });
});
