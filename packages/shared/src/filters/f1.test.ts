...
import { filterF1Event, filterF1Events } from "./f1";
import type { F1Event, F1EventCompetition } from "../types/f1";
...
function makeEvent(competitions: F1EventCompetition[] = [makeCompetition()]): F1Event {
  return {
    $ref: "https://ref",
    id: "evt1",
    date: FUTURE_DATE,
...
describe("filterF1Event", () => {
  it("returns event unchanged when all types included and showPastEvents is true", () => {
    const result = filterF1Event(makeEvent(), { showPastEvents: true, types: ALL_TYPES });
  ...
  it("keeps competition whose type.id is in the types filter", () => {
    const result = filterF1Event(
      makeEvent([makeCompetition({ type: { id: "3", text: "Race", abbreviation: "Race" } })]),
  ...
  it("removes competition whose type.id is not in the types filter", () => {
    const result = filterF1Event(
      makeEvent([makeCompetition({ type: { id: "1", text: "Practice", abbreviation: "P" } })]),
  ...
  it("applies both time and type filters: future+type3 survives, past+type3 fails time, future+type1 fails type", () => {
    const event = makeEvent([
      makeCompetition({ id: "a", date: FUTURE_DATE, type: { id: "3", text: "Race", abbreviation: "Race" } }),
      makeCompetition({ id: "b", date: PAST_DATE, type: { id: "3", text: "Race", abbreviation: "Race" } }),
      makeCompetition({ id: "c", date: FUTURE_DATE, type: { id: "1", text: "Practice", abbreviation: "P" } }),
    ]);
    const result = filterF1Event(event, { showPastEvents: false, types: ["3"] });
    expect(result).not.toBeNull();
    expect(result!.competitions).toHaveLength(1);
  ...
  it("returns null when all competitions are filtered out", () => {
    const event = makeEvent([
      makeCompetition({ type: { id: "1", text: "Practice", abbreviation: "P" } }),
    ]);
...
describe("filterF1Events", () => {
  it("drops events with no remaining competitions", () => {
    ...
    const result = filterF1Events([practice, race], { showPastEvents: true, types: ["3"] });
  ...
  it("returns all events when nothing is filtered", () => {
    ...
    expect(filterF1Events(events, { showPastEvents: true, types: ALL_TYPES })).toHaveLength(2);
  });

  it("returns empty array when input is empty", () => {
    expect(filterF1Events([], { showPastEvents: true, types: ALL_TYPES })).toEqual([]);
  });
});
